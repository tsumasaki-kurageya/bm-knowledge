...
- 「ビルメンテナンス会社の業務」ではなく、建物維持保全を成立させる業務を主体横断で説明する。
- 主な読者は、建物維持保全の実務を知らない開発者とする。
- 読者が仕事を理解するために必要な説明を本文に置く。作成経緯、調査過程、編集上の都合は、内容の根拠や限界を理解するために必要な場合を除いて本文へ持ち込まない。
- 一般的な業務、特定現場の事実、そこからの解釈を混ぜない。分からないことを補完せず「未確認事項」にする。
- 既存の永続 ID、Frontmatter、根拠への参照を維持する。表現を易しくするために正確さや適用条件を落とさない。
- 見出しやページを増やす前に、読者がどこから入り、次にどこへ進むかを確認する。
...
- リンク、永続 ID、Frontmatter、情報源、未確認事項が維持されている。
- `npm run build` が成功する。
