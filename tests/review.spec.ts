...
test('legacy URL keeps query and Japanese heading; document metadata and diagrams work', async ({ page }) => {
  ...
  const link = page.locator('a[href="#追加詳細への入口"]:visible').first();
  await link.click();
  await expect(page.locator('#追加詳細への入口')).toBeInViewport();
...
test('mobile reading, navigation and horizontal table scrolling', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { level: 1 })).toContainText('ライフサイクル');
});
